...
void MakeDensity::compute_energyDensity_3D_distribution(
    ...
    for (auto &string_i : QCD_string_output_arr_) {
        ...
        double Estring =
            mass * (cosh(y_i_l) + cosh(y_i_r) - cosh(y_l) - cosh(y_r));
        ...
        for (int i = 0; i < gridNeta_; i++) {
            ...
            if (eta_arr[i] > eta_l && eta_arr[i] < eta_r) {
                double y_eta =
                    (y_l
                     + (y_r - y_l) / (eta_r - eta_l) * (eta_arr[i] - eta_l));
                feC_arr[i] = cosh(y_eta);
            } else if (
                eta_arr[i] > eta_r && (eta_arr[i] - eta_r) < sigmaDisEta) {
                double dis = eta_arr[i] - eta_r;
                feC_arr[i] = exp(-dis * dis / two_sigma_eta_sq) * cosh(y_r);
            } else if (
                eta_arr[i] < eta_l && (eta_l - eta_arr[i]) < sigmaDisEta) {
            ...
            if (etaDis < sigmaDisEta) {
                feR_arr[i] =
                    exp(-etaDis * etaDis / two_sigma_eta_sq) * cosh(y_r);
            }
            etaDis = std::abs(eta_arr[i] - eta_l);
            if (etaDis < sigmaDisEta) {
                feL_arr[i] =
                    exp(-etaDis * etaDis / two_sigma_eta_sq) * cosh(y_l);
            }
            feCNorm += feC_arr[i];
            feLNorm += feL_arr[i];
            feRNorm += feR_arr[i];
        ...
        for (int k = 0; k < gridNeta_; k++) {
            ...
            double xT =
                (xPerpC
                 + stringTransverseShiftFrac_ * (0.5 - etaFrac)
                       * (xPerpL - xPerpR) / 2.);
            double yT =
                (yPerpC
                 + stringTransverseShiftFrac_ * (0.5 - etaFrac)
...
void MakeDensity::outputTATBEccentricity(
    ...
    for (int iorder = 1; iorder <= orderMax_; iorder++) {
        ...
        fileNameDressed << filenameHeader << "_" << iorder << "_TATB_Nconfig_"
                        << Nconfig << ".dat";
...
void MakeDensity::output_eccentricity(
    std::string filenameHeader, const int eventId, const int outputMode) const {
    ...
    if (outputMode == 0) {
        for (int iorder = 1; iorder <= orderMax_; iorder++) {
            std::ofstream outFile;
            std::stringstream fileNameDressed;
            fileNameDressed << filenameHeader << "_" << iorder << "_Neta_"
                            << gridNeta_ << ".dat";
            outFile.open(fileNameDressed.str().c_str(), modes);
            if (eventId == 0) {
                for (int i = 0; i < gridNeta_; i++) {
        ...
        std::ofstream outFile;
        std::stringstream fileNameDressed;
        fileNameDressed << filenameHeader << "_Neta_" << gridNeta_ << ".dat";
        outFile.open(fileNameDressed.str().c_str(), std::ios::out);
        for (int i = 0; i < gridNeta_; i++) {
            outFile << std::scientific << std::setprecision(5) << eta_arr[i]
                    << "   ";
            for (int iorder = 1; iorder <= orderMax_; iorder++) {
                int idx = (iorder - 1) * gridNeta_ + i;
                outFile << std::scientific << std::setprecision(5)
                        << eccnReal[idx] << "   " << eccnImag[idx] << "   ";
            }
            outFile << std::endl;
        }
        outFile.close();
...
void MakeDensity::output_energyDensity_3d(
    std::string filenameHeader, const int eventId) const {
    std::vector<float> x_arr, y_arr, eta_arr, ed_arr;
    compute_energyDensity_3D_distribution(x_arr, y_arr, eta_arr, ed_arr);
    // output results
    ...
    std::ofstream outFile;
    std::stringstream fileNameDressed;
    fileNameDressed << filenameHeader << "_Neta_" << gridNeta_ << "_Nx_"
                    << gridNx_ << "_Ny_" << gridNy_ << ".dat";
    outFile.open(fileNameDressed.str().c_str(), modes);
    if (eventId == 0) {
        for (int i = 0; i < gridNeta_; i++) {
...
void MakeDensity::output_energyDensity_xeta_distribution(
    ...
    std::ofstream outFile;
    std::stringstream fileNameDressed;
    fileNameDressed << filename << "_Neta_" << gridNeta_ << "_Nx_" << gridNx_
                    << ".dat";
    outFile.open(fileNameDressed.str().c_str(), modes);
    if (eventId == 0) {
        for (int i = 0; i < gridNeta_; i++) {
...
void MakeDensity::output_energyDensity_eta_distribution(
    ...
    for (auto &string_i : QCD_string_output_arr_) {
        ...
        for (int i = 0; i < gridNeta_; i++) {
            ed_arr[i] +=
                (Estring / (feCNorm * gridDeta_) * feC_arr[i]
    ...
    if (outputMode == 0) {
        ...
        for (int i = 0; i < gridNeta_; i++) {
            outFile << std::scientific << std::setprecision(4) << eta_arr[i]
                    << "   " << ed_arr[i] << std::endl;
        }
    }
    outFile.close();
...
void MakeDensity::output_netBaryon_eta_distribution(
    std::string filename, const int eventId, const int outputMode) const {
