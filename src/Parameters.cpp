...
real Parameters::get_shadowing_factor() const {
    real shadowing =
        static_cast<real>(get_param_double("shadowing_factor", 1.0));
    assert(shadowing >= 0.);
    assert(shadowing <= 1.);
    return (shadowing);
}
...
real Parameters::get_yloss_param_fluct_var_LHC() const {
    real a =
        static_cast<real>(get_param_double("yloss_param_fluct_var_LHC", 0.6));
    assert(a >= 0.);
    return (a);
}
...
real Parameters::get_yloss_param_fluct_var_RHIC() const {
    real a =
        static_cast<real>(get_param_double("yloss_param_fluct_var_RHIC", 0.6));
    assert(a >= 0.);
    return (a);
}
...
real Parameters::get_tau_form_mean() const {
    real tau_form_mean =
        static_cast<real>(get_param_double("tau_form_mean", 0.5));
...
real Parameters::get_tau_form_fluct_gamma_beta() const {
    real tau_form_beta =
        static_cast<real>(get_param_double("tau_form_fluct_gamma_beta", 1.0));
