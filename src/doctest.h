...
// https://github.com/philsquared/Catch which uses the Boost Software License -
// Version 1.0 see here -
// https://github.com/philsquared/Catch/blob/master/LICENSE.txt
//
...
#define DOCTEST_MSVC                                    \
    DOCTEST_COMPILER(                                   \
        _MSC_VER / 100, (_MSC_FULL_VER / 100000) % 100, \
...
// - effect C4986 - exception specification does not match previous declaration
...
// '#if/#elif' C4365 - conversion from 'int' to 'unsigned long', signed/unsigned
// mismatch C4774 - format string expected in argument 'x' is not a string
// literal C4820 - padding in structs
...
// https://msdn.microsoft.com/en-us/library/hh567368.aspx GCC C++11 feature
// support table: https://gcc.gnu.org/projects/cxx-status.html MSVC version
// table: MSVC++ 15.0 _MSC_VER == 1910 (Visual Studio 2017) MSVC++ 14.0 _MSC_VER
// == 1900 (Visual Studio 2015) MSVC++ 12.0 _MSC_VER == 1800 (Visual Studio
// 2013) MSVC++ 11.0 _MSC_VER == 1700 (Visual Studio 2012) MSVC++ 10.0 _MSC_VER
// == 1600 (Visual Studio 2010) MSVC++ 9.0  _MSC_VER == 1500 (Visual Studio
// 2008) MSVC++ 8.0  _MSC_VER == 1400 (Visual Studio 2005)
...
// namespaces vs c++98) so the <iosfwd> header is used - also it is very light
// and doesn't drag a ton of stuff
#include <iosfwd>
...
// is an identifier (no spaces and not a C string) - example without quotes:
// I_am_a_message taken from here: http://stackoverflow.com/a/1980156/3162383
...
#define DOCTEST_STATIC_ASSERT(expression, message)                           \
    struct DOCTEST_CAT(__static_assertion_at_line_, __LINE__) {              \
        doctest::detail::static_assert_impl::StaticAssertion<                \
            ...
            DOCTEST_CAT(                                                     \
                DOCTEST_CAT(                                                 \
                    DOCTEST_CAT(STATIC_ASSERTION_FAILED_AT_LINE_, __LINE__), \
                    _),                                                      \
                message);                                                    \
    };                                                                       \
...
class DOCTEST_INTERFACE String {
  ...
  public:
    ...
    // inlined in the vector class somewhere... see commit
    // 574ef95f0cd379118be5011704664e4b5351f1e0 and build
...
DOCTEST_INTERFACE std::ostream& operator<<(
    std::ostream& stream, const String& in);
...
template <
    typename T1 = detail::NullType, typename T2 = detail::NullType,
...
struct DOCTEST_INTERFACE Result {
    ...
    DOCTEST_NOINLINE Result(
        bool passed = false, const String& decomposition = String())
...
struct ExpressionDecomposer {
    ...
    // will stay for now... https://github.com/philsquared/Catch/issues/870
    // https://github.com/philsquared/Catch/issues/565
    template <typename L>
    Expression_lhs<const DOCTEST_REF_WRAP(L)> operator<<(
        const DOCTEST_REF_WRAP(L) operand) {
        return Expression_lhs<const DOCTEST_REF_WRAP(L)>(
            operand, m_assert_type);
    }
};
...
DOCTEST_INTERFACE void logAssert(
    bool passed, const char* decomposition, bool threw, const String& exception,
    const char* expr, assertType::Enum assert_type, const char* file, int line);

DOCTEST_INTERFACE void logAssertThrows(
    bool threw, const char* expr, assertType::Enum assert_type,
    const char* file, int line);

DOCTEST_INTERFACE void logAssertThrowsAs(
    bool threw, bool threw_as, const char* as, const String& exception,
    const char* expr, assertType::Enum assert_type, const char* file, int line);

DOCTEST_INTERFACE void logAssertNothrow(
    bool threw, const String& exception, const char* expr,
...
struct DOCTEST_INTERFACE ResultBuilder {
    ...
    DOCTEST_NOINLINE void binary_assert(
...
template <int comparison, typename L, typename R>
DOCTEST_NOINLINE int fast_binary_assert(
    assertType::Enum assert_type, const char* file, int line, const char* expr,
    const DOCTEST_REF_WRAP(L) lhs, const DOCTEST_REF_WRAP(R) rhs) {
    ...
    // FAILING ASSERTION THIS IS THE EFFECT OF HAVING
...
template <typename L>
DOCTEST_NOINLINE int fast_unary_assert(
    assertType::Enum assert_type, const char* file, int line,
...
// call to operator<< with std::ostream passed as a reference noting that there
// is a use of an undefined type (which there isn't)
DOCTEST_INTERFACE void writeStringToStream(
...
DOCTEST_INTERFACE void toStream(
    std::ostream* stream, int long long unsigned in);
#endif  // DOCTEST_CONFIG_WITH_LONG_LONG
...
class ContextBuilder {
    ...
    void build(std::ostream* stream) const {
        ...
        while (curr < numCaptures
               && curr < DOCTEST_CONFIG_NUM_CAPTURES_ON_STACK)
            reinterpret_cast<const ICapture*>(stackChunks[curr++].buf)
                ->toStream(stream);
        // iterate over list
        Node* curr_elem = head;
        while (curr < numCaptures) {
            reinterpret_cast<const ICapture*>(curr_elem->chunk.buf)
                ->toStream(stream);
...
class DOCTEST_INTERFACE Context {
    ...
    void parseArgs(
        int argc, const char* const* argv, bool withDefaults = false);
...
#define DOCTEST_TEST_CASE_FIXTURE(c, decorators)    \
    DOCTEST_IMPLEMENT_FIXTURE(                      \
        DOCTEST_ANONYMOUS(_DOCTEST_ANON_CLASS_), c, \
        DOCTEST_ANONYMOUS(_DOCTEST_ANON_FUNC_), decorators)
...
#define DOCTEST_TEST_CASE_TEMPLATE_IMPL(decorators, T, types, anon)      \
    ...
    inline int DOCTEST_CAT(anon, REG_FUNC)() {                           \
        ...
        doctest::detail::ForEachType<                                    \
            DOCTEST_HANDLE_BRACED_VA_ARGS(types)::Result,                \
...
#define DOCTEST_TEST_CASE_TEMPLATE(decorators, T, types) \
    DOCTEST_TEST_CASE_TEMPLATE_IMPL(                     \
        decorators, T, types, DOCTEST_ANONYMOUS(_DOCTEST_ANON_TMP_))
#endif  // DOCTEST_CONFIG_WITH_VARIADIC_MACROS
...
#define DOCTEST_ADD_AT_IMPL(type, file, line, mb, x)        \
    do {                                                    \
        doctest::detail::MessageBuilder mb(                 \
            file, line, doctest::detail::assertType::type); \
        mb << x;                                            \
...
#define DOCTEST_TEST_CASE_FIXTURE(x, name)          \
    DOCTEST_IMPLEMENT_FIXTURE(                      \
        DOCTEST_ANONYMOUS(_DOCTEST_ANON_CLASS_), x, \
        DOCTEST_ANONYMOUS(_DOCTEST_ANON_FUNC_), name)
...
// turned on 'Invalid read of size 4' when the test suite len (with '\0') is not
// a multiple of 4 for details see http://stackoverflow.com/questions/35671155
...
struct ContextState : TestAccessibleContextState  //! OCLINT too many fields
{
    ...
    std::vector<IContextScope*>
        contexts;  // for logging with INFO() and friends
...
String& String::operator+=(const String& other) {
    ...
    if (isOnStack()) {
        if (total_size < len) {
            ...
            detail::my_memcpy(
                temp, buf, my_old_size);  // skip the +1 ('\0') for speed
        ...
        if (data.capacity > total_size) {
            ...
            detail::my_memcpy(
                temp, data.ptr, my_old_size);  // skip the +1 ('\0') for speed
            // release old chunk
            delete[] data.ptr;
...
bool operator==(double lhs, Approx const& rhs) {
    ...
    return std::fabs(lhs - rhs.m_value)
           < rhs.m_epsilon
                 * (rhs.m_scale
                    + detail::my_max(std::fabs(lhs), std::fabs(rhs.m_value)));
}
...
// do when empty)
bool matchesAny(
    const char* name, const std::vector<String>& filters, int matchEmpty,
    bool caseSensitive) {
...
Subcase::Subcase(const char* name, const char* file, int line)
    ...
    if (s->subcasesCurrentLevel < s->subcase_filter_levels) {
        if (!matchesAny(
                m_signature.m_name, s->filters[6], 1, s->case_sensitive))
            return;
...
Subcase::Subcase(const Subcase& other)
    : m_signature(
          other.m_signature.m_name, other.m_signature.m_file,
...
void logTestStart(const TestCase& tc) {
    ...
    printToDebugConsole(
        String(getSeparator()) + loc + forDebugConsole.c_str() + n1 + n2
        + subcaseStuff.c_str() + "\n");
}
...
void logTestException(const String& what, bool crash) {
    ...
    printToDebugConsole(
        String(msg) + info1 + info2 + contextStr.c_str() + "\n");
}
...
void logAssert(
    ...
    char loc[DOCTEST_SNPRINTF_BUFFER_LENGTH];
    DOCTEST_SNPRINTF(
        loc, DOCTEST_COUNTOF(loc), "%s(%d)", fileForOutput(file),
        lineForOutput(line));

    char msg[DOCTEST_SNPRINTF_BUFFER_LENGTH];
    DOCTEST_SNPRINTF(
        msg, DOCTEST_COUNTOF(msg), " %s!\n",
        passed ? "PASSED" : getFailString(assert_type));

    char info1[DOCTEST_SNPRINTF_BUFFER_LENGTH];
    DOCTEST_SNPRINTF(
        info1, DOCTEST_COUNTOF(info1), "  %s( %s )\n",
    ...
    if (threw) {
        ...
        DOCTEST_SNPRINTF(
            info3, DOCTEST_COUNTOF(info3), "  %s( %s )\n",
    ...
    printToDebugConsole(
        String(loc) + msg + info1 + info2 + info3 + context.c_str() + "\n");
}

void logAssertThrows(
    bool threw, const char* expr, assertType::Enum assert_type,
    const char* file, int line) {
    char loc[DOCTEST_SNPRINTF_BUFFER_LENGTH];
    DOCTEST_SNPRINTF(
    ...
    if (!threw)
        DOCTEST_SNPRINTF(
            info2, DOCTEST_COUNTOF(info2), "didn't throw at all\n");

    const bool isWarn = assert_type & assertType::is_warn;
    DOCTEST_PRINTF_COLORED(loc, Color::LightGrey);
    DOCTEST_PRINTF_COLORED(
        msg, threw    ? Color::BrightGreen
             : isWarn ? Color::Yellow
                      : Color::Red);
    DOCTEST_PRINTF_COLORED(info1, Color::Cyan);
    DOCTEST_PRINTF_COLORED(info2, Color::None);
...
void logAssertThrowsAs(
    ...
    char info1[DOCTEST_SNPRINTF_BUFFER_LENGTH];
    DOCTEST_SNPRINTF(
        info1, DOCTEST_COUNTOF(info1), "  %s( %s, %s )\n",
    ...
    if (!threw) {  //! OCLINT inverted logic
        ...
        DOCTEST_SNPRINTF(
            info2, DOCTEST_COUNTOF(info2), "threw a different exception:\n");
        DOCTEST_SNPRINTF(
            info3, DOCTEST_COUNTOF(info3), "  %s\n", exception.c_str());
    }
    ...
    DOCTEST_PRINTF_COLORED(loc, Color::LightGrey);
    DOCTEST_PRINTF_COLORED(
        msg, threw_as ? Color::BrightGreen
             : isWarn ? Color::Yellow
                      : Color::Red);
    DOCTEST_PRINTF_COLORED(info1, Color::Cyan);
    DOCTEST_PRINTF_COLORED(info2, Color::None);
...
bool ResultBuilder::log() {
    ...
    if (m_assert_type
        ...
        m_assert_type &  //! OCLINT bitwise operator in conditional
    ...
    if (m_failed || contextState->success) {
        ...
        if (m_assert_type
            & assertType::is_throws) {  //! OCLINT bitwise operator in
                                        //! conditional
            ...
            assertType::is_throws_as) {
            logAssertThrowsAs(
                m_threw, m_threw_as, m_exception_type, m_exception, m_expr,
                m_assert_type, m_file, m_line);
        } else if (
            m_assert_type &  //! OCLINT bitwise operator in conditional
            assertType::is_nothrow) {
            logAssertNothrow(
                m_threw, m_exception, m_expr, m_assert_type, m_file, m_line);
        } else {
            logAssert(
                m_result.m_passed, m_result.m_decomposition.c_str(), m_threw,
...
bool MessageBuilder::log() {
    ...
    char loc[DOCTEST_SNPRINTF_BUFFER_LENGTH];
    DOCTEST_SNPRINTF(
        loc, DOCTEST_COUNTOF(loc), "%s(%d)", fileForOutput(m_file),
    ...
    for (int i = argc - 1; i >= 0; --i) {
        ...
        if (temp && my_strlen(temp) == my_strlen(pattern)) {
            ...
            bool noBadCharsFound =
                true;  //! OCLINT prefer early exits and continue
            while (temp != argv[i]) {
                if (*--temp != '-') {
...
bool parseOptionImpl(
    int argc, const char* const* argv, const char* pattern, String& res) {
    for (int i = argc - 1; i >= 0; --i) {
        const char* temp = std::strstr(argv[i], pattern);
...
bool parseCommaSepArgs(
    ...
    if (parseOption(argc, argv, pattern, filtersString)) {
        ...
        char* pch =
            std::strtok(filtersString.c_str(), ",");  // modifies the string
        while (pch != 0) {
...
void printSummary() {
    ...
    if (p->count || p->list_test_cases) {
        DOCTEST_PRINTF_COLORED("[doctest] ", Color::Cyan);
        std::printf(
            "unskipped test cases passing the current filters: %u\n",
        ...
        const bool anythingFailed =
            p->numFailed > 0 || p->numFailedAssertions > 0;
        ...
        DOCTEST_PRINTF_COLORED(buff, Color::None);
        DOCTEST_SNPRINTF(
            buff, DOCTEST_COUNTOF(buff), "%6u failed", p->numFailed);
        DOCTEST_PRINTF_COLORED(
            buff, p->numFailed > 0 ? Color::Red : Color::None);
        ...
        if (p->no_skipped_summary == false) {
            const int numSkipped =
                ...
                - p->numTestsPassingFilters;
            DOCTEST_SNPRINTF(
                buff, DOCTEST_COUNTOF(buff), "%6d skipped", numSkipped);
            DOCTEST_PRINTF_COLORED(
                buff, numSkipped == 0 ? Color::None : Color::Yellow);
        }
        DOCTEST_PRINTF_COLORED("\n", Color::None);
        ...
        DOCTEST_SNPRINTF(
            buff, DOCTEST_COUNTOF(buff), "assertions: %6d", p->numAssertions);
        DOCTEST_PRINTF_COLORED(buff, Color::None);
        DOCTEST_SNPRINTF(buff, DOCTEST_COUNTOF(buff), " | ");
        DOCTEST_PRINTF_COLORED(buff, Color::None);
        DOCTEST_SNPRINTF(
            buff, DOCTEST_COUNTOF(buff), "%6d passed",
            p->numAssertions - p->numFailedAssertions);
        DOCTEST_PRINTF_COLORED(
            buff, (p->numAssertions == 0 || anythingFailed) ? Color::None
                                                            : Color::Green);
        DOCTEST_SNPRINTF(buff, DOCTEST_COUNTOF(buff), " | ");
        DOCTEST_PRINTF_COLORED(buff, Color::None);
        DOCTEST_SNPRINTF(
            buff, DOCTEST_COUNTOF(buff), "%6d failed", p->numFailedAssertions);
        DOCTEST_PRINTF_COLORED(
        ...
        DOCTEST_PRINTF_COLORED(
            result, p->numFailed > 0 ? Color::Red : Color::Green);
    }
...
#define DOCTEST_PARSE_STR_OPTION(name, sname, var, default)    \
    if (parseOption(argc, argv, name "=", strRes, default)     \
        || parseOption(argc, argv, sname "=", strRes, default) \
        || withDefaults)                                       \
    p->var = strRes
...
int Context::run() {
    ...
    if (!testArray.empty()) {
        if (p->order_by.compare("file", true) == 0) {
            std::qsort(
                &testArray[0], testArray.size(), sizeof(TestCase*),
                fileOrderComparator);
    ...
    for (i = 0; i < testArray.size(); i++) {
        ...
        // execute the test if it passes all the filtering
        {
            ...
            timer.start();
            do {
                // if the start has been logged from a previous iteration of
                // this loop
...
// execute the test
                ...
                if (p->abort_after > 0
                    && p->numFailedAssertions >= p->abort_after) {
                    p->subcasesHasSkipped = false;
                    DOCTEST_PRINTF_COLORED(
            ...
            if (Approx(p->currentTest->m_timeout).epsilon(DBL_EPSILON) != 0
                && Approx(duration).epsilon(DBL_EPSILON)
            ...
            if (p->duration) {
                char msg[DOCTEST_SNPRINTF_BUFFER_LENGTH];
                DOCTEST_SNPRINTF(
                    msg, DOCTEST_COUNTOF(msg), "%.6f s: %s\n", duration,
            ...
            if (data.m_should_fail) {
                ...
                if (p->numFailedAssertionsForCurrentTestcase
                    ...
                    DOCTEST_SNPRINTF(
                        msg, DOCTEST_COUNTOF(msg),
                        "Failed exactly %d times as expected so marking it as "
                        "not failed!\n",
                        data.m_expected_failures);
                    ...
                    DOCTEST_SNPRINTF(
                        msg, DOCTEST_COUNTOF(msg),
                        "Didn't fail exactly %d times so marking it as "
                        "failed!\n",
                        data.m_expected_failures);
                    DOCTEST_PRINTF_COLORED(msg, Color::Red);
                }
            ...
            if (failed)  // if any subcase has failed - the whole test case has
                         // failed
                p->numFailed++;
