...
void QCDString::evolve_QCD_string_with_free_streaming() {
    ...
    y_f_left = y_i_left;
    y_f_right = y_i_right;
    eta_s_left = get_freestreaming_eta_f(
        tau_form, y_i_left, x_production[0], x_production[3]);
    eta_s_right = get_freestreaming_eta_f(
        tau_form, y_i_right, x_production[0], x_production[3]);
}

void QCDString::evolve_QCD_string_with_constant_deceleration() {
    real y_i_lrf = std::abs(y_i_right - y_i_left) / 2.;
    real deceleration_factor =
        (tau_form * tau_form / (2. * m_over_sigma * m_over_sigma));
    ...
    eta_s_left = get_constant_decelerate_eta_f(
        -m_over_sigma, tau_form, y_i_left, x_production[0], x_production[3]);
    eta_s_right = get_constant_decelerate_eta_f(
        m_over_sigma, tau_form, y_i_right, x_production[0], x_production[3]);
}

real QCDString::get_freestreaming_eta_f(
...
real QCDString::get_constant_decelerate_eta_f(
    ...
    const real t_f =
        t_0 + delta_tau * (-temp * sinh_y + sqrt(temp * temp + 1) * cosh_y);
    const real z_f =
        z_0 + delta_tau * (-temp * cosh_y + sqrt(temp * temp + 1) * sinh_y);
    const real eta_s_f = 0.5 * log((t_f + z_f) / (t_f - z_f));
    return (eta_s_f);
}
